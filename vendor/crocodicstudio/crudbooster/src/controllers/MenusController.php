...
class MenusController extends CBController
{
    ...
    public function getIndex()
    {
        ...
        $id_cms_privileges = Request::get('id_cms_privileges');
        //shandy
        //$id_cms_privileges = Request::get('cms_menus_privileges');
        //shandy
        $id_cms_privileges = ($id_cms_privileges) ?: CRUDBooster::myPrivilegeId();

        //shandy
        $roots = cms_menu::whereNull('parent_baum')
        ...
        foreach ($roots as $key => $value) {
            $rootTree[] = cms_menu::select('*')
                ->whereNull('deleted_at')
                ->where('is_active', 1)
                ->where('id', $roots[$key]->id)
                ->orderby('sorting', 'asc')
                ->first()
                ...
                //->get();
                ;
            //$rootTree[] = $a->getDescendants();
            //$a = cms_menu::where('is_active', 1);
    ...
    public function hook_before_add(&$postdata)
    {
        ...
        //shandy
        //if (! $postdata['cms_menus_privileges']) {
            $postdata['id_cms_privileges'] = CRUDBooster::myPrivilegeId();
        //} else
        //{
        //    $postdata['id_cms_privileges'] = $postdata['cms_menus_privileges'];
        //}
        //shandy
    ...
    {
        cms_menu::rebuild(true);
        $post = Request::input('menus');
        $isActive = Request::input('isActive');
        $post = json_decode($post, true);

        $this->saveAMenu($post[0], 0, $isActive);

        $this->rebuildTree();
        return response()->json(['success' => true]);
    }

    public function saveAMenu($menus, $parent_id, $isActive)
    {
        $i = 1;
        foreach ($menus as $ro) {
            $pid = $ro['id'];
            if (count($ro['children'][0]) > 0) {
                $this->saveAMenu($ro['children'][0], $pid, $isActive);
            }
            DB::table('cms_menus')->where('id', $pid)->update(['sorting' => $i, 'parent_id' => $parent_id, 'is_active' => $isActive]);
            $i++;
        }
    }

    public function rebuildTree(){
        DB::statement("UPDATE cms_menus SET parent_baum = parent_id"); 
        cms_menu::where('parent_baum',0)
            ->update(['parent_baum' => null]);
        cms_menu::rebuild(true);
    }
    //shandy

    public function postSaveMenuOld()
    {
        $post = Request::input('menus');
        $isActive = Request::input('isActive');
        $post = json_decode($post, true);
    ...
    public function hook_after_add($id) {        
        DB::table('cms_menus')->where('id', $id)->update(['id_cms_privileges' => Request::get('cms_menus_privileges')[0]]);
    }
