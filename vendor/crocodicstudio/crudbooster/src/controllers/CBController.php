...
//                                 $relationship_table_pk => CRUDBooster::newId($ro['relationship_table']),
    ...
    public function getDelete($id)
    {
        ...
        if (CRUDBooster::isColumnExists($this->table, 'deleted_at')) {
            ...
            DB::table($this->table)->where($this->primary_key, $id)->update(['deleted_at' => date('Y-m-d H:i:s'),'deleted_by' => CB::myId()]);
            //shandy
        } else {
                ...
                if (CRUDBooster::isForeignKey($colname)) {
                    ...
                    if (intval($value->$s)) {
                        ...
                        if (CRUDBooster::isColumnExists($relation_table, 'created_at')) {
                            ...
                            //shandy
                            $relation_insert_data['created_by'] = CB::myId();
                            //shandy
                        }
    ...
    public function postActionSelected()
    {
        ...
        if ($button_name == 'delete') {
            ...
            if (CRUDBooster::isColumnExists($this->table, 'deleted_at')) {
                ...
                DB::table($this->table)->whereIn($tablePK, $id_selected)->update(['deleted_at' => date('Y-m-d H:i:s'),'deleted_by' => CB::myId()]);
                //shandy
            } else {
