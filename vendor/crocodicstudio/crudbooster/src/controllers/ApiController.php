...
class ApiController extends Controller
{
        ...
        */
        ...
        if ($action_type == 'list' || $action_type == 'detail' || $action_type == 'delete') {
            ...
            if ($action_type == 'list') {
                ...
                if (CRUDBooster::isColumnExists($table, 'deleted_at')) {
                    //shandy
                    //Because we use deleted_by
                    $delete = $data->update([
                        $table.'.deleted_at' => date('Y-m-d H:i:s'),
                        $table.'.deleted_by' => $posts['deleted_by']
                    ]);
                    //shandy
                } else {
                    $delete = $data->delete();
    ...
    //shandy
    //Additional method for API Controller
    public function check(&$result) {
        if(isset($this->date)){
            foreach ($this->date as $datekey) {
                if(array_key_exists('data', $result))
                {
                    foreach($result['data'] as $key => $val)
                        if(!empty($result['data'][$key]->$datekey))
                            $result['data'][$key]->$datekey = date('d M Y', strtotime($result['data'][$key]->$datekey));
                    } else {
                        if(!empty($result[$datekey]))
                            $result[$datekey] = date('d M Y', strtotime($result[$datekey]));
                    }
            }            
        }
