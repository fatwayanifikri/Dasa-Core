...
class CRUDBooster
{
    ...
    */
    ...
    public static function myUnitIDKeep()
    {
    ...
    public static function myUnitId()
    {
        ...
        $unitID = DB::table('cms_users')->select('Unit_id')->where('id',$idLogin)->value('Unit_id');
       return $unitID;
        
    }
    //getidcompany
    public static function myCompanyIDKeep()
    {
        return Session::get('admin_company');
    ...
    {
         $idLogin= self::myId();
        $EmployeeId = DB::table('cms_users')->select('Employee_id')->where('id',$idLogin)->value('Employee_id');
        $companyId=DB::table('hrde200_employee')->select('Company_id')->where('id',$EmployeeId)->value('Company_id');
        return $companyId;
    ...
    public static function sidebarDashboard()
    {
        ...
        $menu = DB::table('cms_menus')->whereRaw("cms_menus.id IN (select id_cms_menus from cms_menus_privileges where id_cms_privileges = '".self::myPrivilegeId()."')")->whereNull('deleted_at')->where('is_dashboard', 1)->where('is_active', 1)->first();
    ...
    {
        //shandy
        $roots = cms_menu::whereRaw("cms_menus.id IN (select id_cms_menus from cms_menus_privileges where id_cms_privileges = '".self::myPrivilegeId()."')")
            ->whereNull('parent_baum')
        ...
        foreach ($roots as $key => $value) {
            $rootTree[] = cms_menu::whereRaw("cms_menus.id IN (select id_cms_menus from cms_menus_privileges where id_cms_privileges = '".self::myPrivilegeId()."')")
                ->where('is_dashboard', 0)
                ->whereNull('deleted_at')
                ->where('is_active', 1)
                ->where('cms_menus.id', $roots[$key]->id)
                ->select('cms_menus.*')
                ->orderby('sorting', 'asc')
                ->first()
        ...
        //echo "<br/><br/><br/>";
        $shandymenu = [];
        if(is_array($rootTree)){
            foreach ($rootTree as $m) {
                foreach ($m as $key => $value) {
                    self::getMenuURL($m[$key]);
                    $shandymenu[] = $m[$key];
                }
            }
        }
        ...
        //foreach ($menu as $m) {
        //    echo json_encode($m["170191109203429621"]->name);
        //    echo "<br/><br/>";
        //}
        //shandy
    ...
    //shandy
    private static function getMenuURL(&$c)
    {
        try {
            switch ($c->type) {
                case 'Route':
        ...
        if(!empty($c->children))
        {
            foreach ($c->children as $key => $value) {
                self::getMenuURL($c->children[$key]);
            }
        }
    }
    //shandy
...
//     }
    ...
    {
        //shandy
        $exception = ['id', 'created_at', 'created_by', 'updated_at', 'updated_by', 'deleted_at', 'deleted_by'];
...
<?php namespace App\Http\Controllers;
    ...
    */
    ...
    {
        $pk = CRUDBooster::findPrimaryKey('hrdm106_tipeidentitas');	
			$identitas = DB::table('hrdm106_tipeidentitas')
					->select('id','NamaID')
					->orderBy($pk,'asc')
					->get();
			return $identitas;
    ...
    {
        $pk = CRUDBooster::findPrimaryKey('hrdm102_departement');
			$departement = DB::table('hrdm102_departement')
							->select('id','DepartementName')
							->orderBy($pk,'Desc')
							->get();
			return $departement;
    }
    ...
    {
        $pk = CRUDBooster::findPrimaryKey('hrdm100_company');
			$company = DB::table('hrdm100_company')
    ...
    {
        $pk = CRUDBooster::findPrimaryKey('hrdm101_unit');	
			$unit = DB::table('hrdm101_unit')
					->select('id','UnitName')
					->orderBy($pk,'asc')
					->get();
    ...
    {
        $pk = CRUDBooster::findPrimaryKey('hrdm103_level');
			$level = DB::table('hrdm103_level')
					->select('id','LevelName')
					->orderBy($pk,'ASC')
					->get();
			return $level;
    ...
    {
        $pk = CRUDBooster::findPrimaryKey('hrdm107_educationlevel');
            $education = DB::table('hrdm107_educationlevel')
    ...
    public static function getStatusKaryawan()
    {
        $pk = CRUDBooster::findPrimaryKey('hrdm108_employeestatus');
        $statusKaryawan = DB::table('hrdm108_employeestatus')
    ...
    {
        if(Session::get('sessionTime'))
            if(time() > Session::get('sessionTime') + 10)
            {
                $users = $users = DB::table(config('crudbooster.USER_TABLE'))->where("id", Session::get('admin_id'))->first();
                ...
                //adittesting session
                $unitID = DB::table('cms_user')->where("id",$users->id);
                ...
                Session::put('admin_unit',$UnitID->Unit_id);
            }
    }
    //shandy
}
