...
<h3>{{$k->where('Unit_id', '=', 2)->sum('qty_produksi')}}</h3>
@break
@empty
<h3>0</h3>
@endforelse
...
<h3>{{$v->where('status_produksi', '=', 2)->count()}}</h3>
@break
@empty
<h3>0</h3>
@endforelse
...
<div class="inner">
@forelse($produksi as $v)
<h3>{{$v->where('status_produksi', '=', 3)->count()}}</h3>
@break
@empty
<h3>0</h3>
@endforelse
...
QTY Total : {{ number_format($grand_total) }} <br/>
