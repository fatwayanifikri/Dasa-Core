...
<h3>{{number_format($k->where('mesin_id', '=', 7)->where('tgl_produksi', '=', date('Y-m-d') )->where('status_produksi', '<', 3)->sum('qty_produksi'))}}</h3>
@break
@empty
<h3>0</h3>
@endforelse
...
<div class="inner">
@forelse($produksi as $k)
<h3>{{number_format($k->where('mesin_id', '=', 6)->where('tgl_produksi', '=', date('Y-m-d') )->where('status_produksi', '<', 3)->sum('qty_produksi'))}}</h3>
@break
@empty
<h3>0</h3>
@endforelse
...
<td><button type="button" title="Clear" onclick="location.href='../admin/produksi_monitoring'"><i class="fa fa-backward"></i></button></td>
</tr>
...
<td>{{number_format($p->mesin->kapasitas_produksi)}}</td>
<td>{{number_format($p->qty)}}</td>
<td>{{number_format($p->mesin->kapasitas_produksi - $p->kapasitas)}}</td>
<td>

<a href="{{ url("admin/produksi_view?tgl_produksi=".$p->tgl_produksi."&mesin_id=".$p->mesin_id) }}" title="Detail">
<center><i class="fa fa-book fa-lg" style="color:orange;"></i></center></a>  
