<?php namespace App\Http\Controllers;
	...
	class AdminMutasiAssetLogistikController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->form[]= ['label'=>'Kode Asset','name'=>'asset_id','type'=>'datamodal','datamodal_table'=>'loga001_asset','datamodal_columns'=>'kode,nama','datamodal_columns_alias'=>'kode,name','datamodal_select_to'=>'nama:nama,kategori_id:kategori_id,Unit_id:asal_unit','datamodal_size'=>'small','required'=>true];
			...
			$this->form[] = ['label'=>'Asal Unit','name'=>'asal_unit','type'=>'select','validation'=>'required|min:1|max:255','width'=>'col-sm-2','datatable'=>'hrdm101_unit,UnitName','readonly'=> true];
     ...
     //        $asal=DB::table('loga004_mutasiasset')
		    ...
		    $baru=DB::table('loga004_mutasiasset')
				...
				// if ($getJabatan == 1 ){
				// 	$this->addaction[] = ['label'=>'Setujui','name'=>'setuju_sm','url'=>('ApproveSMasal').'/[id]','icon'=>'fa fa-check','color'=>'success','showIf'=>"[isApprove] == 0 "];
				// 	$this->addaction[] = ['label'=>'Tolak','name'=>'tolak_sm','url'=>('Rejectmutasiaset').'/[id]','icon'=>'fa fa-check','color'=>'danger','showIf'=>"[isApprove] == 0"];
...
//dd($asal);
	    ...
	    */
	    public function hook_query_index(&$query) {
		    ...
		    elseif ($getUnitID == 2) {
		  	if($getJabatan == 12){
						$query
						->where('loga004_mutasiasset.asal_unit','=',$getUnitID);
       ...
       public function ApproveSMasal($id){
			
	
				DB::table('loga004_mutasiasset')
				->where('id','=',$id)
				->update(['isApprove'=>'1']);
		 ...
		 public function ApproveSMtujuan($id){
			
			$Unit= DB::table('loga004_mutasiasset')
				   ->where('id','=',$id)
				   ->value('unit_tujuan');
	        $id_asset= DB::table('loga004_mutasiasset')
				   ->where('id','=',$id)
				   ->value('asset_id');
		...
		public function Rejectmutasiaset($id){
			
          DB::table('loga004_mutasiasset')
		  ->where('id','=',$id)
          ->update(['isApprove'=>'3']);
          CRUDBooster::redirect($_SERVER['HTTP_REFERER'],"Request Mutasi Berhasil di Reject!","info");
}
