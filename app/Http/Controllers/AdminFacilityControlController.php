<?php namespace App\Http\Controllers;
	...
	class AdminFacilityControlController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
	      ...
	      ->where('id', \DB::raw("(select max(`id`) from hgst100_reqmaintenance)"))
	        ...
	        */
	        ...
	        // $this->index_button[] = ['label'=>'Print xlsx','url'=>('downloadExcelGS/xlsx?'),'icon'=>'fa fa-file-excel-o','color'=>'info'];
         //    $this->index_button[] = ['label'=>'Print pdf','url'=>('PrintGS'),'icon'=>'fa fa-file-pdf-o','color'=>'danger'];
	        $this->index_button[] = ['label'=>'Print','url'=>('FacilityControlCustom'),'icon'=>'fa fa-print','color'=>'primary'];
	    ...
	    */
	    ...
	    //-----------------------------Untuk Filter Tanggal Di table hgst101_reqmaintenancedetail----------//    
	    $id=DB::Table('hgst100_reqmaintenance')
	    ->where('id', \DB::raw("(select max(`id`) from hgst100_reqmaintenance)"))
	    ->value('id');

	     $date=DB::Table('hgst100_reqmaintenance')
	    ->where('id', \DB::raw("(select max(`id`) from hgst100_reqmaintenance)"))
	    ->value('tgl_request');

	    DB::table('hgst101_reqmaintenancedetail')->where('requestmaintenance_id','=',$id)->update(['created_at'=>$date]);
...
DB::table('hgst101_reqmaintenancedetail AS detail')
->select('detail.requestmaintenance_id')
->leftJoin('hgst100_reqmaintenance AS request','request.id','=','detail.requestmaintenance_id')
->whereNull('request.id')->delete();

	    }

// 	public function PrintGS()
// 		{
// 			$dataprintgs = DB::table('hgst100_reqmaintenance as request')
// 							->select([
// 								'tgl_request',
// 								'unit.UnitName as namaunit',
// 								'detail.defect as defect',
// 								'category.kategoriname as kategori',
// 								'detail.lokasi as lokasi',
// 								'detail.kategori_area as kategori_area',
// 								'detail.actionplan as actionplan',
// 								'detail.reasoning as reasoning',
// 								'detail.work_status as work_status',
// 								'detail.remarks as remarks',
								
// 							])
// 							->join('hgst101_reqmaintenancedetail AS detail','detail.requestmaintenance_id','=','request.id')
// 							->join('hrdm101_unit AS unit','unit.id','=','request.Unit_id')
// 							->join('hgst102_reqmaintenancekategori AS category','category.id','=','detail.kategori')
...
// 		$query = DB::table('hgst100_reqmaintenance as request')
// 							->select([
// 								'tgl_request',
// 								'unit.UnitName as namaunit',
// 								'detail.defect as defect',
