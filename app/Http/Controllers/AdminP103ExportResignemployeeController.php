<?php namespace App\Http\Controllers;
	...
	use DB;
	use CRUDBooster;
	use App\Exportresign;
	use crocodicstudio\crudbooster\controllers\partials\ButtonColor;
    use Excel;
	class AdminP103ExportResignemployeeController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
           ...
           //-----------------CONTROLLER EXPORT EMPLOYEE RESIGN---------------------//
			...
			$this->col[] = ["label"=>"TanggalKeluar","name"=>"TanggalKeluar"];
			...
			//$this->form[] = ["label"=>"TanggalKeluar","name"=>"TanggalKeluar","type"=>"date","required"=>TRUE,"validation"=>"required|date"];
			# OLD END FORM
...
$query = \App\Models\p103_resignemployee::when(request('Unit_id'), function($query){
$query->whereHas('unit', function($nested){
$nested->where('Unit_id', request('Unit_id'));/*query untuk search field berelasi*/
   	});
   })
->when(request('from') && request('until'), function($q){ 	
$q->whereBetween('TanggalKeluar', [request('from'), request('until')]);
})
->paginate()
...
$data = [];
$data['p103_resignemployee'] = 'Products Data';
$data['result'] = $query;
$data['value'] = $query2;
    
   //Create a view. Please use `cbView` method instead of view method from laravel.
$this->cbView('viewindex/custom_employee_resign',$data);
}
...
/*-------------------EXPORT EXCEL--------------------*/
public function importExport6()
	{
		return view('costum_resign');
	}
	public function downloadExcel6($type)
	{
...
return Excel::create('Data_Resign', function($excel) use ($query) {
$excel->sheet('mySheet', function($sheet) use ($query)
	        {
$sheet->loadView('exports.resign', array('data' => $query));
});
})->download($type);
	}

public function importExcel6()
	{
