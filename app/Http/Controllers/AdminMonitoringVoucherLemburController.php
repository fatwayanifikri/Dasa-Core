...
use PDF;
	...
	class AdminMonitoringVoucherLemburController extends \crocodicstudio\crudbooster\controllers\CBController {
...
$q->whereBetween('tgl_pengajuan_voucher', [request('from2'), request('until2')]);
})
...
->paginate(10)
->appends(request()->query()); //supaya filter tidak reset pas ganti page
...
$this->cbView('viewindex/custom_monitoring_voucherlembur',$data);
}
/*-------------------------------END COSTUM VIEW--------------------*/

public function importExportVoucher4()
{
return view('custom_lembur_export');
}
public function downloadExcelVoucher4($type)
{
...
});//load viewnya pake export pengajuan voucher punya HC
})
->download($type);
}

public function importExcelVoucher4()
	{
...
public function printvoucher4pdf()
		{
...
$generatePDF = PDF::loadView('exports.PrintVoucherLemburHCpdf',array('query'=>$query))->setPaper('a4','landscape'); //load viewnya pake export pengajuan voucher punya HC
