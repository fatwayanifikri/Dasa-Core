...
use Auth;
	class AdminListUsers71Controller extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->form[] = ['label'=>'Cms Privileges','name'=>'id_cms_privileges','type'=>'select2','validation'=>'required|min:1|max:255','width'=>'col-sm-10','datatable'=>'cms_privileges,name'];
...
//untuk query pencarian dan ambil data dari database.
   $query = DB::table('cms_users')
   ->when(request('name'), function($query){
   		$query->where('name', 'LIKE', '%'. request('name') .'%');/*query untuk search field nonrelasi*/
   })
   ->orderby('id','desc')
   ->paginate(10000);
   ...
   $data = [];
   $data['editusers'] = 'Products Data';
   $data= $query;
   //Create a view. Please use `cbView` method instead of view method from laravel.
   $this->cbView('costum_list_user',$data);
}
...
public function edituser($id)
{
	...
	// passing data ticket yang didapat ke form report
