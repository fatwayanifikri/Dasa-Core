<?php namespace App\Http\Controllers;
	...
	use DB;
	use CRUDBooster;
	use App\Models\loga002_riwayatasset;
	use PDF;
	use Excel;


	class AdminAssetLogistikCustomController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->table = "loga002_riwayatasset";
			...
			$this->col[] = ["label"=>"Tgl Pemakaian","name"=>"tgl_pemakaian"];
			$this->col[] = ["label"=>"Tgl Pembelian","name"=>"tgl_pembelian"];
			...
			$this->form[] = ['label'=>'Tgl Pemakaian','name'=>'tgl_pemakaian','type'=>'date','validation'=>'required|date','width'=>'col-sm-10'];
			$this->form[] = ['label'=>'Tgl Pembelian','name'=>'tgl_pembelian','type'=>'date','validation'=>'required|date','width'=>'col-sm-10'];
			...
			//$this->form[] = ["label"=>"Tgl Pemakaian","name"=>"tgl_pemakaian","type"=>"date","required"=>TRUE,"validation"=>"required|date"];
			//$this->form[] = ["label"=>"Tgl Pembelian","name"=>"tgl_pembelian","type"=>"date","required"=>TRUE,"validation"=>"required|date"];
			# OLD END FORM
...
if($getUnitID == 1){
   ...
   $query = \App\Models\loga002_riwayatasset::when(request('Unit_id'), function($query){
    ...
    $q->whereBetween('tgl_pemakaian', [request('from'), request('until')]);
   })

     ->when(request('from2') && request('until2'), function($q){ 
    $q->whereBetween('tgl_pembelian', [request('from2'), request('until2')]);
   })
   ...
   ->where('loga002_riwayatasset.Unit_id',$getUnitID)
   ->orderBy('id','desc')
   ->paginate()
   ->appends(request()->query());
}
  ...
  $query3 = DB::table('cms_users')
   ...
   $this->cbView('viewindex/custom_assetlogistik',$data);
}
...
if($getUnitID == 1){
   ...
   ->get();
			...
			$generatePDF = PDF::loadView('exports.PrintAssetpdf',array('query'=>$query))->setPaper('a4','potrait');
...
/* ------------------------------EXPORT EXCL---------------------------------- */
	    ...
	    public function importExportAsset()
	{
		return view('Facility_Control');

	}
	public function downloadExcelAsset($type)
	{
