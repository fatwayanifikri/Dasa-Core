...
use PDF;
	...
	class AdminExportVoucherLemburController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			//$this->form[] = ["label"=>"Shift","name"=>"shift","type"=>"text","required"=>TRUE,"validation"=>"required|min:1|max:255"];
...
//SUPERADMIN BISA MELIHAT SEMUA DATA VOUCHER//
...
//HC HANYA BISA MELIHAT VOUCHER LEMBUR HO (MENU LEMBUR HO)//

elseif($getJabatan == 62 || $getJabatan == 63 || $getJabatan == 64 || $getJabatan == 65 || $getJabatan == 66){
...
//ADMIN HANYA BISA MELIHAT VOUCHER YG SUDAH DI BUAT DAN DITERIMA//

else{

$query = \App\Models\t112_absenlembur::when(request('UnitName'), function($query){
$query->whereHas('unit', function($nested){
...
$this->cbView('viewindex/custom_lemburvoucher_export',$data);
}
/*-------------------------------END COSTUM VIEW--------------------*/

//----------------------------EXPORT PDF--------------------------//

public function printvoucherpdf()
		{
...
$generatePDF = PDF::loadView('exports.PrintVoucherLemburpdf',array('query'=>$query))->setPaper('a4','landscape');
...
public function importExportVoucher()
{
return view('custom_lembur_export');
}
public function downloadExcelVoucher($type)
{
...
return Excel::create('Voucher_Lembur', function($excel) use ($query) {
$excel->sheet('mySheet', function($sheet) use ($query)
	        {
$sheet->loadView('exports.PrintVoucherLemburexcel', array('data' => $query));
});
})
->download($type);
}

public function importExcelVoucher()
	{
