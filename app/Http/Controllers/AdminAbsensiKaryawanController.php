<?php namespace App\Http\Controllers;
	...
	class AdminAbsensiKaryawanController extends \crocodicstudio\crudbooster\controllers\CBController {
...
public function getIndex() {
 ...
 if(!CRUDBooster::isView()) CRUDBooster::redirect(CRUDBooster::adminPath(),trans('crudbooster.denied_access'));
   ...
   $query = \App\Models\hrde200_employee::when(request('Jabatan_id'), function($query){
	$query->whereHas('jabatan', function($nested){
  ...
  ->get();
   ...
   $this->cbView('viewindex/custom_add_absensinew',$data);
}
...
public function mulailembur(Request $request)
{
    ...
    $endtime = Carbon::now();
	 ...
	 ->insert(['Employee_id'   => $request->Employee_id,
	 	       'tanggal_absen' =>  $datenow,
	 	       'mulai_lembur'=> $endtime]);

	 redirect('admin/copylembur')->send();
}


//---------COPY DATA BARU KE LAMA---------//

public function copylembur(){
  ...
  $getpreviousdata = DB::table('t202_absensi')
  ->where('Employee_id','=',$getid)
  ->where('tanggal_absen','=',$gettgl)
  ->get();

  foreach($getpreviousdata as $q){

  	if($q->absen_pulang == null){
     
     DB::table('t202_absensi')
	->where('id', \DB::raw("(select max(`id`) from t202_absensi)"))
	->delete();

	CRUDBooster::redirect($_SERVER['HTTP_REFERER'],"Belum Absen Pulang!","danger");
	...
	$getlembur=DB::table('t202_absensi')
					->where('id', \DB::raw("(select max(`id`) from t202_absensi)"))
					->value('mulai_lembur');
	...
	DB::table('t202_absensi')
		   ...
		   ->where('tanggal_absen', $gettgl)
           ->update([
            'mulai_lembur' =>$getlembur,
           ]);
redirect('admin/deletelembur')->send();
...
public function deletelembur()
{
	...
	CRUDBooster::redirect($_SERVER['HTTP_REFERER'],"Absen Lembur Berhasil!","success");
}
...
public function addselesailembur()
{ 
	
	return view('selesailembur');
}
...
public function selesailembur(Request $request)
{
    ...
    $endtime = Carbon::now();
	 ...
	 ->insert(['Employee_id'   => $request->Employee_id,
	 	       'tanggal_absen' =>  $datenow,
	 	       'selesai_lembur'=> $endtime]);

	 redirect('admin/copyselesailembur')->send();
}


//---------COPY DATA BARU KE LAMA---------//

public function copyselesailembur(){
  ...
  foreach($getpreviousdata as $q){
  	...
  	if($q->mulai_lembur == null){
     
     DB::table('t202_absensi')
	->where('id', \DB::raw("(select max(`id`) from t202_absensi)"))
	->delete();

	CRUDBooster::redirect($_SERVER['HTTP_REFERER'],"Belum Absen Lembur!","danger");
	...
	$getselesai=DB::table('t202_absensi')
					->where('id', \DB::raw("(select max(`id`) from t202_absensi)"))
	...
	DB::table('t202_absensi')
		   ...
		   ->where('tanggal_absen', $gettgl)
           ->update([
            'selesai_lembur' =>$getselesai,
           ]);
redirect('admin/deleteselesailembur')->send();
}
}
}

public function deleteselesailembur()
{
	DB::table('t202_absensi')
	->where('EmployeeName','=',null)
	->delete();
