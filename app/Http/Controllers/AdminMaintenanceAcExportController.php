<?php namespace App\Http\Controllers;
	...
	class AdminMaintenanceAcExportController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->form[] = ['label'=>'Nama Ac','name'=>'nama_ac','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
			$this->form[] = ['label'=>'Unit Id','name'=>'Unit_id','type'=>'select2','validation'=>'required|integer|min:0','width'=>'col-sm-10','datatable'=>'Unit,id'];
			$this->form[] = ['label'=>'Kode Mb','name'=>'kode_mb','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
			...
			$this->form[] = ['label'=>'Periode','name'=>'periode','type'=>'number','validation'=>'required|integer|min:0','width'=>'col-sm-10'];
			$this->form[] = ['label'=>'Jadwal Pertama','name'=>'jadwal_pertama','type'=>'datetime','validation'=>'required|date_format:Y-m-d H:i:s','width'=>'col-sm-10'];
			...
			//$this->form[] = ["label"=>"Jadwal Pertama","name"=>"jadwal_pertama","type"=>"datetime","required"=>TRUE,"validation"=>"required|date_format:Y-m-d H:i:s"];
			# OLD END FORM
...
->when(request('kode_mb'), function($query){
$query->where('kode_mb', 'LIKE', '%'. request('kode_mb') .'%');
})
->orderby('id','desc')
->paginate(10)
...
$this->cbView('viewindex/custom_AC_export',$data);
}


public function printACallpdf()
		{

$query = \App\Models\hgst109_maintenance_ac_detail::when(request('Unit_id'), function($query){
$query->whereHas('unit', function($nested){
$nested->where('Unit_id', request('Unit_id'));});
})
->when(request('from') && request('until'), function($q){ 
