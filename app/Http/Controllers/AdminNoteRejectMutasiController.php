<?php namespace App\Http\Controllers;
	...
	class AdminNoteRejectMutasiController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->form[] = ['label'=>'Unit Asal','name'=>'AsalUnit_id','type'=>'select','validation'=>'required|min:1|max:255','width'=>'col-sm-4','datatable'=>'hrdm101_unit,UnitName','value'=>$getUnitID,'readonly'=>true];
			$this->form[] = ['label'=>'Alasan Reject','name'=>'reject_note','type'=>'textarea','validation'=>'required|min:1|max:255','width'=>'col-sm-4'];
			...
			//$this->form[] = ["label"=>"TanggalMutasi","name"=>"TanggalMutasi","type"=>"date","required"=>TRUE,"validation"=>"required|date"];
			//$this->form[] = ["label"=>"AsalUnit Id","name"=>"AsalUnit_id","type"=>"select2","required"=>TRUE,"validation"=>"required|min:1|max:255","datatable"=>"AsalUnit,id"];
			//$this->form[] = ["label"=>"AsalPrivileges Id","name"=>"AsalPrivileges_id","type"=>"select2","required"=>TRUE,"validation"=>"required|min:1|max:255","datatable"=>"AsalPrivileges,id"];
			//$this->form[] = ["label"=>"Unit Id","name"=>"Unit_id","type"=>"select2","required"=>TRUE,"validation"=>"required|min:1|max:255","datatable"=>"Unit,id"];
			//$this->form[] = ["label"=>"Privileges Id","name"=>"Privileges_id","type"=>"select2","required"=>TRUE,"validation"=>"required|min:1|max:255","datatable"=>"Privileges,id"];
       ...
       if ($dashboard && $dashboard->url) {
        return redirect('admin/Mutasi%20Karyawan')->send();
        exit;
         }
