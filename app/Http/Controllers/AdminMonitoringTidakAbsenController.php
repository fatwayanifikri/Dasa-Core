<?php namespace App\Http\Controllers;
	...
	class AdminMonitoringTidakAbsenController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->col[] = ["label"=>"Is Approve","name"=>"is_approve"];
			...
			$this->form[] = ['label'=>'Jam Pelaksanaan','name'=>'jam_pelaksanaan','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
			$this->form[] = ['label'=>'Keterangan','name'=>'keterangan','type'=>'textarea','validation'=>'required|string|min:5|max:5000','width'=>'col-sm-10'];
			$this->form[] = ['label'=>'Is Approve','name'=>'is_approve','type'=>'radio','validation'=>'required|integer','width'=>'col-sm-10','dataenum'=>'Array'];
...
$query = \App\Models\t204_formtidakabsen::when(request('EmployeeName','Unit_id'), function($query){
...
$q->whereBetween('tanggal', [request('from'), request('until')]);
})
...
$this->cbView('viewindex/custom_tidak_absen',$data);
}

//------------------------------PDF-----------------------
public function print_tidakabsenALL(){
...
$generatePDF = PDF::loadView('exports.PrintFormTidakAbsenALLpdf',array('query'=>$query))->setPaper('a4','landscape');
...
public function downloadExcelTidakAbsen($type)
{
...
return Excel::create('Data_Tidak_Absen', function($excel) use ($query) {
$excel->sheet('mySheet', function($sheet) use ($query){
$sheet->loadView('exports.PrintFormTidakAbsenALLexcel', array('data' => $query));
});
})->download($type);
return back();
}
...
public function delete_form1($id)
{
