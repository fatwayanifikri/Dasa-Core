...
use PDF;
	...
	class AdminExportVoucherFinanceController extends \crocodicstudio\crudbooster\controllers\CBController {
...
$q->whereBetween('tgl_pencairan_voucher', [request('from'), request('until')]);
})
...
->when(request('tgl_pencairan_voucher'), function($query){
$query->where('tgl_pencairan_voucher', 'LIKE', '%'. request('tgl_pencairan_voucher') .'%');
})
->where('isVoucher','=','3')
->orderby('id','desc')
->paginate()
...
$this->cbView('viewindex/custom_lemburvoucherfinance_export',$data);
}
/*-------------------------------END COSTUM VIEW--------------------*/

public function importExportVoucher2()
{
return view('custom_lembur_export');
}
public function downloadExcelVoucher2($type)
{
...
$excel->sheet('mySheet', function($sheet) use ($query)
	        {
$sheet->loadView('exports.PrintVoucherLemburFinancexcel', array('data' => $query));
});
})
->download($type);
}

public function importExcelVoucher2()
	{
...
public function printvoucher2pdf()
		{
...
$generatePDF = PDF::loadView('exports.PrintVoucherLemburFinancepdf',array('query'=>$query))->setPaper('a4','landscape');
