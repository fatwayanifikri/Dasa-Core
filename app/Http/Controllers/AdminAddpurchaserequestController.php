<?php namespace App\Http\Controllers;
	...
	class AdminAddpurchaserequestController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
            ...
            $ResultID = DB::select('select rand()*100 as id');
			...
			$this->form = [];
			$this->form[] =['label'=>'id','name'=>'id','type'=>'hidden','value'=>$ResultID[0]->id];
			$this->form[] = ['label'=>'NoPR','name'=>'NoPR','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10','value'=>$kodePR,'readonly'=>true];
			...
			// isstatus => 0=open, 1= close
			$this->form[] = ['label'=>'IsApproved','name'=>'IsApproved','type'=>'hidden','value'=>'0'];
			// isstatus => 0=belum disetujui. 1=disetujui

			$columns = [];
			$columns[] = ['label'=>'id','name'=>'purchaserequest_id','type'=>'hidden','value'=>$ResultID[0]->id];
			$columns[] = ['label'=>'Kode Barang','name'=>'barang_id','type'=>'datamodal','datamodal_table'=>'logm101_baranginventory','datamodal_columns'=>'Kodebarang,NamaBarang','datamodal_select_to'=>'NamaBarang:NamaBarang','datamodal_where'=>'','datamodal_size'=>'small'];
...
public function getIndex() {
  //First, Add an auth
   ...
   $query = DB::table('logt201_purchaserequest')
            ->orderby('id','desc')
            ->paginate(10);
   ...
   $this->cbView('costum_addpurchaserequest',$data);
}
