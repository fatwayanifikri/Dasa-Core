<?php namespace App\Http\Controllers;
	...
	class AdminT112AbsenlemburvoucherfinanceController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->col[] = ["label"=>"Jumlah Voucher","name"=>"JumlahVoucher"];
			$this->col[] = ["label"=>"Nilai Voucher","name"=>"NilaiVoucher"];
			$this->col[] = ["label"=>"Tgl Pencairan","name"=>"tgl_pencairan_voucher"];
		...
		//	$this->form[] = ['label'=>'Departement','name'=>'Departement_id','type'=>'select2','validation'=>'required|min:1|max:255','width'=>'col-sm-10','datatable'=>'hrdm102_departement,DepartementName'];
			...
			$this->form[] = ['label'=>'Status Voucher','name'=>'isVoucher','type'=>'hidden','value'=>'1'];
...
// 			// 	$this->addaction[] = ['label'=>'Buat Voucher','url'=>('t112_absenlemburvoucher/edit').'/[id]','icon'=>'fa fa-check','color'=>'success','showIf'=>"[AmountMinute] > 299 & [isApproved] == 1 & [isVoucher] == 2" ];
// 			// }
...
// }
			// //-------------------------Button Cair---------------------------------//
			if(CRUDBooster::MyPrivilegeId()==62){
				$this->addaction[] = ['label'=>'Cairkan','url'=>('vouchercair').'/[id]','icon'=>'fa fa-check','color'=>'success','showIf'=>"[isApproved] == 1 & [isVoucher] == 2"];
			}
            ...
            elseif(CRUDBooster::MyPrivilegeId()==48){
				$this->addaction[] = ['label'=>'Cairkan','url'=>('vouchercair').'/[id]','icon'=>'fa fa-check','color'=>'success','showIf'=>"[isApproved] == 1 & [isVoucher] == 2"];
			}
	        ...
	        */
	        ...
	        | 
	        */
	        $this->index_button[] = ['label'=>'Export','url'=>('export_voucher_finance'),'icon'=>'fa fa-download','color'=>'primary'];
	        ...
	        */
			...
			$this->table_row_color[] = ['condition'=>"[isVoucher]==3","color"=>"success"];
	        $this->table_row_color[] = ['condition'=>"[isVoucher]==2","color"=>"warning"];
	        /*
	        | ---------------------------------------------------------------------- 
	...
	//	dd($getJabatan);
	        //Your code here
	        if($getJabatan == 48){
				$query->whereIn('isVoucher',[2,3])
				->orderBy('isVoucher','asc')
				->get();
			}
			elseif($getJabatan == 49){
				$query->whereIn('isVoucher',[2,3])
				->orderBy('isVoucher','asc')
				->get();
			}
			elseif($getJabatan == 50){
				$query->whereIn('isVoucher',[2,3])
				->orderBy('isVoucher','asc')
				->get();
			}
			elseif($getJabatan == 51){
				$query->whereIn('isVoucher',[2,3])
				->orderBy('isVoucher','asc')
				->get();
			}
