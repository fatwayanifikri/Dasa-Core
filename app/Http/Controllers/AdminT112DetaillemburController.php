...
use Excel;
	...
	class AdminT112DetaillemburController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->col[] = ["label"=>"EmployeeName","name"=>"EmployeeName","join"=>"hrde200_employee,id"];
			...
			$this->form[] = ['label'=>'Employee Id','name'=>'Employee_id','type'=>'select2','validation'=>'required|min:1|max:255','width'=>'col-sm-10','datatable'=>'Employee_id,id'];
			...
			$this->form[] = ['label'=>'EmployeeName','name'=>'EmployeeName','type'=>'select2','validation'=>'required|min:1|max:255','width'=>'col-sm-10','datatable'=>'EmployeeName,id'];
			...
			$this->form[] = ['label'=>'IsApproved','name'=>'isApproved','type'=>'hidden','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
...
//untuk query pencarian relasi dan ambil data dari database.
...
->when(request('from') && request('until'), function($q){ 
   ...
   ->select('Employee_id','EmployeeName','Unit_id','Jabatan_id','Departement_id', DB::raw('SUM(AmountMinute) as total_minute'))
   ->groupby('Employee_id','EmployeeName','Unit_id','Jabatan_id','Departement_id')
 ->where('isApproved','=','1')
   ->orderby('id','desc')
   ->paginate();
//---------------------------------------------------------------------------------------
...
->get();
   ...
   $this->cbView('viewindex/costum_lembur_detail',$data);
}
/*-------------------------------END COSTUM VIEW--------------------*/
...
public function importExport()
   ...
   ->orderby('id','desc')
		   ->get();
  //------------------------------------------------------------------------------------------------	   
		return Excel::create('Rekap_Lembur', function($excel) use ($query) {
			$excel->sheet('mySheet', function($sheet) use ($query)
	        {
				$sheet->loadView('exports.rekaplembur', array('data' => $query));
	        });
		})->download($type);
	}

	
	public function importExcel3()
	{
