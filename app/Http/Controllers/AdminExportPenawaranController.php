<?php namespace App\Http\Controllers;
	...
	class AdminExportPenawaranController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->col[] = ["label"=>"Nomor Penawaran","name"=>"Nomor_Penawaran"];
			$this->col[] = ["label"=>"Perihal","name"=>"Perihal"];
			$this->col[] = ["label"=>"Tgl Request","name"=>"tgl_request"];
			$this->col[] = ["label"=>"CompanyName","name"=>"CompanyName"];
			$this->col[] = ["label"=>"CompanyAddress","name"=>"CompanyAddress"];
			$this->col[] = ["label"=>"CompanyPhoneNumber","name"=>"CompanyPhoneNumber"];
			# END COLUMNS DO NOT REMOVE THIS LINE

			# START FORM DO NOT REMOVE THIS LINE
			$this->form = [];
			$this->form[] = ['label'=>'Customer ID','name'=>'Customer_ID','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
			$this->form[] = ['label'=>'Nomor Penawaran','name'=>'Nomor_Penawaran','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
			...
			//$this->form[] = ["label"=>"Nomor Penawaran","name"=>"Nomor_Penawaran","type"=>"text","required"=>TRUE,"validation"=>"required|min:1|max:255"];
...
//----------------------------------------------------
$query = \App\Models\mkt002_penawaran::when(request('Sales_Unit','SalesID'), function($query){
$query->whereHas('unit', function($nested){
$nested->where('Sales_Unit', 'LIKE', '%'. request('Sales_Unit') .'%');
 });
$query->whereHas('sales', function($nested){
$nested->where('SalesID', 'LIKE', '%'. request('SalesID') .'%');
 });
 })
//----------------------------------------------------

->when(request('from') && request('until'), function($q){ 
$q->whereBetween('tgl_request', [request('from'), request('until')]);
})
...
$this->cbView('viewindex/custom_penawaran_export',$data);
}

//-----------------------------------------------------------------------//

public function Printpenawaran_pdf()
		{

//----------------------------------------------------

$printpenawaran = \App\Models\mkt002_penawaran::when(request('Sales_Unit','SalesID'), function($printpenawaran){
$printpenawaran->whereHas('unit', function($nested){
$nested->where('Sales_Unit', 'LIKE', '%'. request('Sales_Unit') .'%');
 });
$printpenawaran->whereHas('sales', function($nested){
$nested->where('SalesID', 'LIKE', '%'. request('SalesID') .'%');
 });
 })
//----------------------------------------------------
...
->select (DB::raw('MONTH(tgl_request) as month'),'Nomor_Penawaran','CompanyName','tgl_request','SalesID','Sales_Unit')
->orderBy('tgl_request','desc')
->get();
 
 $generatePDF = PDF::loadView('exports.PrintPenawaranpdf',array('printpenawaran'=>$printpenawaran))->setPaper('a4','landscape');return $generatePDF->stream();		
